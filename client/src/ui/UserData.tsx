...
export default function UserData() {
  ...
  return (
    <div className="hero-content flex-col lg:flex-row m-6  shadow-sm shadow-primary rounded-lg">
    <img src={user?.profilePic} className="max-w-sm h-32  rounded-xl shadow-2xl" />
    <div>
      <h1 className="text-5xl font-bold">Your profile!</h1>
      <p className="py-2 flex justify-between items-center mt-6"><span className='font-semibold'>Full Name:</span> <span className='uppercase text-sm'>{user?.fullName}</span></p>
      ...
      <div className='flex justify-end mt-6'><EditProfileModal user={user} /></div>
    </div>
  </div>
  )
}
